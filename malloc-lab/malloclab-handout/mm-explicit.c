/*
 * mm-explicit.c - Use explicit free list as the approach.
 * 1. Use an explicit free list to record free blocks.
...
int mm_init(void) {
  ...
  PUT(heap_listp + (4 * WSIZE), PACK(2 * DSIZE, 1)); /* Prologue fotter */
 ...
 */
static void *find_fit(size_t size) {
  void *bp = NEXT_FREE_BLKP(heap_listp, heap_listp);
  while (bp != heap_listp) {
...
/*
 * add_free_block - Add a free block into the explicit free list.
 */
static void add_free_block(void *bp) {
  char *ptr = heap_listp;
  ...
  while ((next_ptr != heap_listp) && (next_size < size)) {
    ptr = next_ptr;
...
/*
 * remove_free_block - Remove a free block from the explicit free list.
 */
static void remove_free_block(void *bp) {
